...
private:
        ...
        while (x != r && !x->getParent()->getIsBlack()) {
            ...
            if (treeIsLeftChild(x->getParent()) == 1) {
                ...
                if (y != nullptr && !y->getIsBlack()) {
                    // 祖父节点的右子节点为红色
                    // 父节点与叔节点改为黑色，祖父节点改为红色（除根节点外），并将指针更新为祖父节点后再次处理红节点连续问题
                    x = x->getParent();
                    x->setIsBlack(true);
                    x = x->getParent();
                    x->setIsBlack(x == r);
                    ...
                    if (treeIsLeftChild(x) == 0) {
                        ...
                        // 这一步的作用是将<号变成/号，使其转换x为父节点的左子节点的情况
                        x = x->getParent();
                        treeLeftRotate(x);
                    }
                    // 将父节点改为黑色，祖父节点改为红色，将以祖父节点为根的子树进行右旋转
                    // 这一步是将/号变成^号，完成平衡
                    x = x->getParent();
                    x->setIsBlack(true);
                    x = x->getParent();
                ...
                if (y != nullptr && !y->getIsBlack()) {
                    ...
                    // 将父节点改为黑色，祖父节点改为红色，将以祖父节点为根的子树进行左旋转
                    // 这一步是将\号变成^号，完成平衡
                    x = x->getParent();
                    x->setIsBlack(true);
                    x = x->getParent();
    ...
    static void treeRemove(Node *r, Node *z) {
        // z指向需要被删除的节点
        // 因为一个节点同时拥有两个子节点时直接删除会导致两个子节点需要争夺一个位置，所以无法直接删除，
        // 需要寻找下一个节点（必定不可能同时拥有两个子节点）来填充原位置
